...
class UserManager
{
    ...
    {
        $login = $User->getLogin();
        $stmt = $this->_db->query("SELECT * FROM utilisateurs WHERE login='$login'");
        $data = $stmt->fetch(PDO::FETCH_ASSOC);
        if ($login == $data['login'] && password_verify($User->getPassword(), $data['password'])) {
    ...
    public function insert(User $User)
    {
        ...
        $pass = password_hash($User->getPassword(), CRYPT_BLOWFISH);
